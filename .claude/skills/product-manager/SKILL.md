...
- **リポジトリルートの README はセットアップ / 利用手順 / メイン公開 API のみ**。リポジトリ全体の索引としての ARCHITECTURE.md は置かない（アーキテクチャの WHY は JSDoc または該当箇所のコードコメントに集約する）。ただし `packages/kamado/ARCHITECTURE.md` のような**個別パッケージ内部の実装解説**は対象外（コントリビューター向けの実装ガイドとして維持してよい）
