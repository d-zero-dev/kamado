...
| `--incremental`    |        | 記録済みの入力が変わっていない出力のコンパイルをスキップ。`.kamado/cache/build-manifest.json` の検証トレースを使用         |
...
ビルドのたびに、出力ファイルごとの検証トレースを記録します。トレースの内容は、コンパイルが読み取ったすべてのファイル（ページ本体・sidecar JSON・レイアウト・pugのinclude・CSSの`@import`・postcss設定・参照画像ファイル・バンドルされたスクリプトのimport）のコンテンツハッシュと、環境ダイジェスト（グローバルデータ・ページリスト・ページアセット一覧・コンパイラオプション・同梱ツールチェーンのバージョン・設定ファイルの内容）です。次の`--incremental`ビルドでは、トレースがすべて一致し出力ファイルが存在する出力について、書き込みだけでなく**コンパイル自体をスキップ**します。すべてコンテンツハッシュベースのため、mtimeに依存せず、`.kamado/cache/`を保持すればマシンをまたいでも機能します。

##### キャッシュの場所と`.gitignore`

マニフェストは設定ファイルのあるディレクトリ（設定ファイルが見つからない場合はカレントディレクトリ）の `.kamado/cache/build-manifest.json` に書き込まれます。これは再生成可能なビルドキャッシュです。

- **推奨**: `.kamado/` をプロジェクトの `.gitignore` に追加してください。古い・不完全なマニフェストが誤ったビルドを生むことはなく（フルビルドに戻るだけ）、コミットする必要はありません。
- **CI**: CI 間で `--incremental` を高速に保つには、コミットではなく CI のキャッシュ機構で `.kamado/cache/` を復元してください（キーは任意）。トレースはコンテンツハッシュベースのため、別マシン・別ランナーで復元しても有効です。
- **クリーンビルドを強制する場合**: `.kamado/cache/` を削除してください（または `--incremental` なしで実行）。
