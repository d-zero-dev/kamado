...
ビルドのたびに、出力ファイルごとの検証トレースを記録します。トレースの内容は、コンパイルが読み取ったすべてのファイル（ページ本体・sidecar JSON・レイアウト・pugのinclude・CSSの`@import`・バンドルされたスクリプトのimport）のコンテンツハッシュと、環境ダイジェスト（グローバルデータ・ページリスト・コンパイラオプション・設定ファイルの内容）です。次の`--incremental`ビルドでは、トレースがすべて一致し出力ファイルが存在する出力について、書き込みだけでなく**コンパイル自体をスキップ**します。すべてコンテンツハッシュベースのため、mtimeに依存せず、`.kamado/cache/`を保持すればマシンをまたいでも機能します。

注意点：

- JavaScript関数の中だけで起きる変更（カスタムtransform・compile hooks・関数を返すJSグローバルデータファイル）はダイジェストから見えません。設定ファイル自体の編集はハッシュされるため検出されます。設定ファイル外でそうしたコードを変更した場合や、kamado・コンパイラパッケージを更新した場合は、`.kamado/cache/`を削除するか一度`--incremental`なしでビルドしてください
