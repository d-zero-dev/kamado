...
| `--incremental`    |        | 記録済みの入力が変わっていない出力のコンパイルをスキップ。検証トレースのマニフェストを使用（デフォルトでは OS の一時ディレクトリにキャッシュ） |
| `--force`          |        | `--incremental` と併用し、既存キャッシュを無視して全再ビルドしたうえでキャッシュを更新する（手動削除の代わり）                                 |
| `--cache-dir <p>`  |        | インクリメンタルビルドのキャッシュディレクトリ。デフォルトは OS の一時ディレクトリ配下のプロジェクト固有フォルダ。相対パスは cwd 基準で解決    |
...
# 前回のビルドから変わったものだけを再コンパイル
kamado build --incremental

# キャッシュを手動削除せずにクリーンな全再ビルド
kamado build --incremental --force

# キャッシュをツリー内に置く（例: CI で復元する）
kamado build --incremental --cache-dir .kamado/cache
```

#### `--incremental`の仕組み

ビルドのたびに、出力ファイルごとの検証トレースを記録します。トレースの内容は、コンパイルが読み取ったすべてのファイル（ページ本体・sidecar JSON・レイアウト・pugのinclude・CSSの`@import`・postcss設定・参照画像ファイル・バンドルされたスクリプトのimport）のコンテンツハッシュと、環境ダイジェスト（グローバルデータ・ページリスト・ページアセット一覧・コンパイラオプション・同梱ツールチェーンのバージョン・設定ファイルの内容）です。次の`--incremental`ビルドでは、トレースがすべて一致し出力ファイルが存在する出力について、書き込みだけでなく**コンパイル自体をスキップ**します。すべてコンテンツハッシュベースのため、mtimeに依存せず、キャッシュディレクトリを保持すればマシンをまたいでも機能します。

##### キャッシュの場所

デフォルトでは、マニフェストは**プロジェクトツリーの外**——OS の一時ディレクトリ配下のプロジェクト固有フォルダ（`<tmp>/kamado/<名前>-<ハッシュ>/build-manifest.json`、設定ファイルのディレクトリで名前空間化）に書き込まれます。そのためデフォルトでは `.gitignore` に追加するものはなく、一時ディレクトリが消えても単にフルビルドになるだけです。キャッシュはいつでも再生成できるビルド成果物で、古い・不完全なマニフェストが誤ったビルドを生むことはありません（フルビルドに戻るだけ）。

- **クリーンな全再ビルドを強制**: `kamado build --incremental --force`（手動削除は不要）。
- **CI 間で永続化**: `--cache-dir` で保存・復元するディレクトリを指定してください（例: `--cache-dir .kamado/cache` とし、`.kamado/` を gitignore してそのパスをキャッシュ）。トレースはコンテンツハッシュベースのため、別マシン・別ランナーで復元しても有効です。
- **ツリー内キャッシュ**: `--cache-dir .kamado/cache` でプロジェクト内に置けます。`.kamado/` を `.gitignore` に追加してください。

注意点：

- JavaScript関数の中だけで起きる変更（カスタムtransform・compile hooks・関数を返すJSグローバルデータファイル）はダイジェストから見えません。設定ファイル自体の編集はハッシュされるため検出されます。設定ファイル外でそうしたコードを変更した場合は、一度 `--force` で実行してください（またはキャッシュを削除）。（同梱の esbuild/postcss/cssnano のアップグレードはダイジェストに自動で織り込まれます。一方 esbuild が参照する `tsconfig.json` は追跡されないため、設定隣接コードと同様に編集後は `--force` で実行してください）
